...
## 第 7 轮 · 长文本从候选栏分离（短语/模板分层）
> 状态：已完成 · commit a4ec1a6 · 2026-08-23

### 问题
- 518 条常用语里约 78 条是 >100 字的长文本/模板
- 这些长文本混入候选栏后，短 key（如 de/ni）会匹配到长文章，打字严重误触
- 用户反馈「打字的时候不要匹配长文本」

### 方案
- 阈值 100 字为界：≤100 字 = 短常用语（phrase），>100 字 = 长文本/模板（snippet）
- 短常用语：正常参与候选栏排序 + 空格自动展开
- 长文本：候选栏完全不出现，只在短语面板里手动浏览点击上屏
- 两端面板加「全部 / 短语 / 长文本」分段切换

### 改动文件
- `HotwordsStore.swift` — isLongText 属性
- `Searcher.swift` — includeLongText 参数，默认 false
- `ios/.../KeyboardViewController.swift` — 短语面板加 segmented control + 过滤逻辑
- `UI/PhrasesPanel.swift` — Mac 面板加分段控制

### 验证结果
- ✅ swift build 通过
- ✅ 引擎回归全绿
- ✅ de 输入：9 条 → 8 条（过滤掉 1 条长文本常用语，还有 1 条短语保留）
- ⚠️ 面板 UI 交互待用户实测

---

## 第 8 轮 · 产品级常用语体验（对齐微信/搜狗）
> 状态：已完成 · commit 4edbd83 · 2026-08-23

### 背景
用户反馈「没有惊艳的产品级输入法」「微信输入法有专门管理常用语的地方」。
此前拍脑袋做功能，没对标主流产品。调研微信/Gboard/搜狗后一次性补齐。

### 调研结论（三家共同点）
- 常用语触发：自定义输入码 + 前 N 字拼音联想（微信/搜狗都是）
- 面板入口：键盘顶部 logo/图标 → 快捷功能面板 → 常用语
- 长文本：和短语混在一起，输入码精确触发，不单独分组
- 撇号键：都在第二行最右（L键右侧），做音节分隔符

### 本轮改动
1. **修复 keyPrefix 分数 bug** — 简码前缀匹配被当成精确匹配给 10000 分，导致 de 输入时长文本排第一
2. **清理脏数据** — 518→410 条（删 102 空 key + 6 短 key 长文本）
3. **5-12 字全拼前缀联想** — 对齐微信「前 N 字拼音联想」，打短语前几个字的拼音就能联想出整条
4. **iOS 键盘顶部 P logo 按钮** — 点击展开快捷面板（常用语/设置入口），对齐微信输入法交互
...
10000 — 常用语 key 精确匹配
 4999 — 词库精确匹配
 3500 — 常用语 key 前缀
 2500 — 常用语 initials 首字母（≤4字）
 1999 — 词库前缀匹配
 1500 — 常用语全拼前缀（5-12字，微信式联想）
  100 — 文本子串包含
```
...
- ✅ swift build 通过
- ✅ 引擎回归全绿
- ✅ de 输入：汉字候选排前，常用语只在末尾（key=dej 前缀匹配，3500 分）
- ✅ wom 输入：首选「我们」（词库前缀）正确，未被常用语挤压
- ⚠️ iOS UI 交互待用户真机实测

---

## 第 9 轮 · iOS 键盘产品级体验补齐
> 状态：已完成 · commit c318a46 · 2026-08-23

### 问题
用户反馈「iOS 输入法还是简陋得很」。此前只有基础打字功能，缺少产品级细节。

### 本轮改动（7 项）
1. **候选栏数字标号** — 格式「1 的」「2 地」，标号 12pt 灰色 + 候选 17pt，对齐微信/Gboard
2. **按键按下高亮** — setBackgroundImage(.highlighted) 半透明底色，按下去有视觉反馈
3. **触觉反馈** — UIImpactFeedbackGenerator(.light)，每个键 touchDown 轻震
4. **删除键长按连续删** — 0.5s 延迟 → 0.1s/个 → 每 5 个加速（最短 0.03s），每 3 个震一次
5. **回车上屏首选** — 有候选上屏第 1 个 / 有拼音上屏拼音 / 空状态换行
6. **双击空格出句号** — 对齐微信/Gboard 标准交互
7. **符号/数字面板切换闭环** — 字母⇄123⇄#+= 三态切换通畅

### 验证结果
- ✅ Mac 端 swift build 通过
- ✅ 引擎回归全绿
- ⚠️ iOS UI 交互待真机实测（无法在 Mac 端编译验证）
...
- ✅ Mac 常用语管理面板（增删改查 + 搜索 + 导入导出 + 短/长分类）
- ✅ iOS 宿主 App 常用语管理（增删改 + 搜索 + 内置种子）
- ✅ 长文本与短常用语分层（候选栏只显示短语，长文本只在面板里）
